...
async function initDB() { await pool.query('CREATE TABLE IF NOT EXISTS tracker_data (key TEXT PRIMARY KEY, value TEXT NOT NULL, updated_at TIMESTAMPTZ DEFAULT NOW())'); await pool.query('CREATE TABLE IF NOT EXISTS session (sid VARCHAR NOT NULL COLLATE "default", sess JSON NOT NULL, expire TIMESTAMP(6) NOT NULL, CONSTRAINT session_pkey PRIMARY KEY (sid)); CREATE IDEX ON session (expire)').catch(()=>{}); console.log('DB ready'); }
function requireAuth(req,res,next){ if(req.session&&req.session.authenticated)return next(); res.redirect('/login'); }
...
app.post('/login',async(req,res)=>{ const {password}=req.body; console.log('Login attempt:', password); const ok=await checkPassword(password); console.log('Password check result:', ok); if(ok){ req.session.authenticated=true; req.session.save((err)=>{ if(err) console.error('Session save error:',err); res.redirect('/'); }); }else{ res.redirect('/login?error=1'); } });
...
initDB().then(()=>{ app.listen(PORT,'0.0.0.0',()=>{ console.log(`LifeBridge Tracker running on port ${PORT}`); }); }).catch(err=>{ console.error('Failed to init DB:',err); process.exit(1); });
